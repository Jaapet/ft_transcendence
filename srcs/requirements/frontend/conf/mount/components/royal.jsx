...
const Royal = () => {
	...
	function main()
	{
		...
		const fov = 45;
		const aspect = 2;  // la valeur par défaut du canvas
		const near = 0.1;
		const far = 10000;
		...
		/// possibiliter de tourner chaque user de son cote
...
*/
		...
		function BallHit(ball, Balls) {
			...
			var hit = [0, 0];
			Balls.forEach(BallToCheck => {
					if (BallToCheck[0] == ball)
							return;
					const BallToCheckBox = new THREE.Box3().setFromObject(BallToCheck[0]);
					if (ballBox.intersectsBox(BallToCheckBox)) {
							const ballCenter = ballBox.getCenter(new THREE.Vector3());
							const BallToCheckCenter = BallToCheckBox.getCenter(new THREE.Vector3());
							const hitVector = ballCenter.clone().sub(BallToCheckCenter);
							hit[0] = hitVector.z;
							hit[1] = hitVector.x;
					}
			});
		...
		function render(time)
		{
			...
			objects.forEach(objtab => {
				// se deplace dans une direction, s'arrete au bord, repart dans l'autre sens
				const obj = objtab[0];
